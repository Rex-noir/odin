...
export class HashSet {
  ...
  //get the key
  get(key) {
    const k = this._map[this.hash(key, this._capacity)].getKey();
    if (k) {
      return k;
    }
    ...
    for (let bucket of this._map) {
      if (bucket) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (value) values.push(node.getKey());
          if (key) keys.push(node.getKey());
          if (entry) entries.push(node.getKey());
        }
      }
    }
    if (key) return keys;
    ...
    for (const bucket of this._map) {
      if (bucket) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          newMap[newIndex].append(node.getKey());
        }
      }
    }
    this._map = newMap;
